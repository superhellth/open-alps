...
## 1. `download_extracts` — `phases/downloads/download_extracts.py`
...
## 2. `filter_trails` — `phases/preprocessing/filter_trails.py`
...
## 3. `merge_trails` — `phases/preprocessing/merge_trails.py`
...
## 4. `verify_trails` — `phases/preprocessing/verify_trails.py`  (gate, always reruns — `uptodate: [False]`)
...
## 5. `fetch_huts` — `phases/downloads/fetch_huts.py`
...
## 5b. `fetch_stations_parking` — `phases/downloads/fetch_stations_parking.py`
...
## 5c. `filter_start_points` — `phases/preprocessing/filter_start_points.py`
...
- `filter_to_hut_range()`: a correct (not approximate) filter — drops every station/parking point
  ...
  `phases/graph_building/build_hub_edges.py`'s real-distance cutoff, regardless of how the trail actually routes — this
...
## 6a. `build_base_graph` — `phases/graph_building/build_base_graph.py`  (the expensive step, ~4.1h for AT+Bayern)
...
cached across hub-set changes and downstream hyperparameter retuning; `phases/graph_building/build_hub_edges.py` (which
...
2. **contract_structural** (`lib/contraction.py`) — collapses every run of degree-2 nodes (pure
   ...
   hub snapping is deferred to `lib/edge_split.py` at `phases/graph_building/build_hub_edges.py` time. Built via
...
3. Nodes are assigned a `lib/grid.py` `Grid` cell id (`config.graph.tileSizeKm`) and re-sorted by
   ...
   `phases/graph_building/build_hub_edges.py`'s workers mmap-slice just their own region instead of loading everything.
...
## 6b. `build_hub_edges` — `phases/graph_building/build_hub_edges.py`  (tiled, multiprocess)
...
- **Output**: `data/osm/hut_edges/{records.npy, geometry.npy}` and
  ...
  `phases/elevation/add_elevation.py`).
...
## 7. `fetch_dem` — `phases/downloads/fetch_dem.py`
...
- Providers (`pipeline/phases/downloads/dem_providers/`):
...
## 7b. `build_dem_vrt` — `phases/elevation/build_dem_vrt.py`
...
- **materialize_geotiff** phase: bakes the (lazily-reprojecting) `dem.vrt` into a real
  ...
  `phases/elevation/add_elevation.py` (rerun often, to retune noise threshold) doesn't keep re-paying that cost.
- **doit wiring**: `file_dep=[fetch_manifest.json]`, `targets=[dem.vrt, dem.tif]`.

## 8. `add_elevation` — `phases/elevation/add_elevation.py`  (always reruns when selected, `uptodate: [False]`; cheap, ~90–100s)
...
## 9. `build_trail_tiles` — `phases/postprocessing/build_trail_tiles.py`
...
## 11. `build_hut_edge_tiles` / `build_start_edge_tiles` — `phases/postprocessing/build_edge_tiles.py`
...
- **`lib/timing.py`** — `phase(script, name, **meta)` context manager, appends one JSON line to
  ...
  misleading partial record). Used by the expensive scripts (`phases/graph_building/build_base_graph.py`,
  `phases/elevation/build_dem_vrt.py`, `phases/elevation/add_elevation.py`) to track which phase stops scaling first as regional
...
- **`lib/grid.py`** — `Grid`, a row-major spatial grid partitioning a bbox into
  ...
  re-derivable identically at both `phases/graph_building/build_base_graph.py` write time and `phases/graph_building/build_hub_edges.py`
...
- **`lib/contraction.py`** — `contract_structural()`, the pure structural (no hub-snap exception)
  chain-contraction function `phases/graph_building/build_base_graph.py` calls.
...
- **`lib/subgraph.py`** — `gather_padded_subgraph()`, the padded-region (cell + buffer) mmap
  gather `phases/graph_building/build_hub_edges.py`'s per-cell workers use to slice the persisted base graph.
