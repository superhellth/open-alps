...
All hyperparameters live in one place: **`data/pipeline.config.json`**.
...
Every script reads this file (via `scripts/lib/pipeline.py`'s `load_config()`) instead of
...
data/
  ...
  scripts/
    ...
    01-09                          # individual steps, in run order — plain Python scripts
  osm/
    raw/
...
python data/scripts/run_all.py --only 7,8          # DEM + elevation only
python data/scripts/run_all.py --only 9            # just rebuild the raw-trail vector tiles
```
...
```bash
python data/scripts/run_all.py --only 6 -- --max-edge-km 15
```
...
```bash
python data/scripts/01-download-extracts.py      # ~1.6GB, Geofabrik extracts from pipeline.config.json
python data/scripts/02-filter-trails.py           # -> ~264MB combined, hiking ways only
python data/scripts/03-merge-trails.py            # -> data/osm/trails.osm.pbf
python data/scripts/04-verify-trails.py           # gate: fails if trails.osm.pbf is missing/empty

python data/scripts/05-fetch-huts.py              # -> data/osm/huts.geojson
